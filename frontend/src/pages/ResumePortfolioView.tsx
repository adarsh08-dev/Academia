...
export const ResumePortfolioView: React.FC<ResumePortfolioViewProps> = ({
  ...
  const displayEmail = customData.email || student?.email || 'adarsh@aiims.edu';
  const displayPhone = customData.phone || '+91 98765 43210';
  const displayLocation = customData.location || 'New Delhi, India';
  const displayCgpa = customData.cgpa || 8.8;
  const displayBio = customData.bio || 'Dedicated medical learner and clinical intern with demonstrated competencies in emergency resuscitation (ACLS/BLS), clinical pharmacology, diagnostic reasoning, and multicenter ICMR clinical research. Cryptographically verified clinical logbooks and ranked in top percentile for clinical readiness.';
