    ...
    <div className="space-y-6 animate-fade-in select-none">
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-r from-[#141C48] via-[#10173F] to-[#0A0F2E] border border-[#1E2B68] flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-black text-white">{localStorage.getItem('userName') || 'Faculty'}</h1>
          <p className="text-xs text-slate-300">
            Head of Department · {localStorage.getItem('userCourse') || 'Department'} · {localStorage.getItem('userCollege') || 'Institution'}
          </p>
        </div>
