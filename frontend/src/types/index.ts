...
export interface CareerLadderStep {
  id: string;
  stepNumber: number;
  stageName: string; // e.g. 'Learning', 'Skills', 'Mentorship', 'Research', 'Internship', 'Industry Exposure', 'Career Opportunities'
  title: string;
  description: string;
...
export interface MedicalCareerPathway {
  id: string;
  title: string;
  specialty: string;
  targetDegree: string; // MBBS, MD/MS, DNB, DM/MCh, PhD, MPH
  overview: string;
  durationYears: string;
  readinessBenchmark: number;
  coreCompetencies: string[];
  keyMilestones: { stage: string; focus: string; duration: string }[];
  recommendedResearch: string[];
  hospitalPlacementRoles: string[];
  industryDemand: 'Critical' | 'High' | 'Emerging';
  averageRemuneration: string;
}
...
export interface ClinicalCaseStudy {
  ...
  demographics: { age: number; gender: string; presentation: string };
  vitalSigns: { hr: string; bp: string; rr: string; temp: string; spo2: string };
  objective: string;
