...
const DEFAULT_MEDICAL_SKILL_BADGES: MedicalSkillBadge[] = [
  { id: 'sk-anatomy', name: 'Anatomy & Physiology', level: 3, icon: '🫀' },
  { id: 'sk-pharmacology', name: 'Pharmacology', level: 2, icon: '💊' },
  { id: 'sk-diagnostics', name: 'Clinical Diagnosis', level: 4, icon: '🩺' },
  { id: 'sk-pathology', name: 'Pathology Basics', level: 3, icon: '🔬' },
  { id: 'sk-ebm', name: 'Evidence-Based Medicine', level: 3, icon: '📋' }
];

const DEFAULT_QUESTS: Quest[] = [
  { id: 'q1', title: 'Review 1 Anatomy Topic', time: '15 min', xp: 50, completed: false },
  { id: 'q2', title: 'Read 1 Medical Research Article', time: '20 min', xp: 60, completed: false },
  { id: 'q3', title: 'Solve 1 Clinical Case Scenario', time: '20 min', xp: 60, completed: false },
  { id: 'q4', title: 'Review 1 Pharmacology Concept', time: '15 min', xp: 40, completed: false }
];
...
export const SkillTwinAndQuests: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const questVersion = localStorage.getItem('medicalQuestsVersion');

    if (savedDate !== today || questVersion !== 'v3') {
      localStorage.setItem('questDate', today);
      localStorage.setItem('medicalQuestsVersion', 'v3');
      ...
      if (savedQuests) {
        try {
          ...
          const isMedical = Array.isArray(parsed) && parsed.length > 0 && parsed.some((q: Quest) => 
            q.title.includes('Anatomy') || q.title.includes('Clinical') || q.title.includes('Medical') || q.title.includes('Pharmacology')
          );
          if (isMedical) {
            setQuests(parsed);
          } else {
    ...
    // Load active medical skills from service if available
    try {
      const storedSkills = getStudentSkills();
      if (storedSkills && storedSkills.length > 0) {
        // Map medical skills to badges
        const medicalIcons: Record<string, string> = {
          'anatomy': '🫀',
          'physiology': '🫀',
          'pharmacology': '💊',
          'diagnostics': '🩺',
        ...
        const mappedBadges: MedicalSkillBadge[] = storedSkills.slice(0, 5).map(sk => {
          const lower = sk.name.toLowerCase();
          let icon = '🩺';
          for (const key of Object.keys(medicalIcons)) {
          ...
          return {
            id: sk.id,
            name: sk.name.length > 22 ? sk.name.split('&')[0].trim() : sk.name,
            level: sk.level || 3,
            icon
          };
        });
        ...
        if (mappedBadges.length > 0) {
          setSkillBadges(mappedBadges);
        }
      }
    } catch {
      setSkillBadges(DEFAULT_MEDICAL_SKILL_BADGES);
    }
  ...
  const title = level <= 3 ? 'Clinical Intern' : level <= 6 ? 'Research Fellow' : 'Chief Resident';
      ...
      <div className="relative bg-gradient-to-br from-[#12162E] to-[#1A1F3D] border border-white/10 rounded-[20px] p-5 overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-5">
          {skillBadges.map((sk) => (
            <div 
              key={sk.id}
              className="text-[11px] font-medium text-white/80 bg-white/5 border border-white/10 px-2.5 py-1 rounded-lg hover:border-[#7C5CFC]/50 hover:bg-[#7C5CFC]/10 transition-colors cursor-default shadow-sm flex items-center gap-1.5"
            >
              <span>{sk.name} — Lv. {sk.level}</span>
              <span>{sk.icon}</span>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-[#0B0F2A]/50 border border-white/5 rounded-xl p-2.5 flex flex-col items-center justify-center">
            <span className="text-[10px] text-white/40 uppercase font-semibold mb-1">Rotations</span>
            <span className="text-sm font-bold text-white">8</span>
          </div>
          <div className="bg-[#0B0F2A]/50 border border-white/5 rounded-xl p-2.5 flex flex-col items-center justify-center">
            <span className="text-[10px] text-white/40 uppercase font-semibold mb-1">Grand Rounds</span>
